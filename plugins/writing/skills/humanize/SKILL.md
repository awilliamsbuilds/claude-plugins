...
### Step 6: Skill Self-Update Proposal
...
2. **Is this a new pattern worth catching in future reviews?** If yes, propose adding it to the appropriate section:
...
### Rules for proposing a new pattern:

- Write it as a specific, flaggable rule with an example
- Identify the correct section of this file where it would be inserted
- Do not duplicate existing rules
- Do not propose vague rules. If you can't give a concrete example from the content just reviewed, don't propose it.

### Output to the user after identifying candidates:

If new patterns were found, present them for review **before making any changes**:

```
## Proposed Skill Update

I found [X] new pattern(s) from this review. Please approve before I add them:

**Pattern 1:** [pattern name]
- Section: [which section it would go in]
- Rule: [the flaggable rule, written as it would appear in the skill]
- Example: [concrete example from the content just reviewed]

[repeat for each pattern]

Reply "approve all", "approve 1, 3" (by number), or "skip" to add none.
```

Do not write any changes to the skill file until the user explicitly approves. After approval, make only the approved additions and confirm what was added.

If no new patterns were found:
