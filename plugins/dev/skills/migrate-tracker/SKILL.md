...
> construction (`references/entry-adapters.md` §A6 defines it, and `dev:pr` Step 5b relies on it);
> this skill is the first to lift one from untrusted
...
>   not cosmetic. A cycle named from a Linear issue is legitimately mixed-case —
>   `references/entry-adapters.md` §A6 documents `^[A-Za-z0-9][A-Za-z0-9-]*$` — so stripping `ENG-123-auth-bug` without lowercasing
