...
**Spec challenger — an errored dispatch is not an iteration.** In loop-bookkeeping terms: a dispatch that returns an error rather than a verdict does **not** advance `challenge.loops_run`, sets `challenge.blockers` / `challenge.concerns` to `null`, and is **retried once — once per stage, not once per round.** A second error STOPs. The per-stage scoping is the part that only matters here: standard mode never loops, so one retry per stage and one per round are indistinguishable there, while a deep-tier autopilot run reading it per-round would grant five retries instead of one. `dev:spec` Step 12a is **canonical** for this rule, including why it is not mode-split, how a standard-mode STOP is recovered, and why an errored dispatch is not the in-session fallback; only the bookkeeping half is restated here.

**Plan challenger: bounded revision loop.** `dev:plan` Step 7a's blockers drive an auto-revision loop capped at `challenge_plan.loops_max` (standard 3 / deep 5 — micro never reaches Plan), re-dispatching on the revised `plan.md` each iteration, incrementing `challenge_plan.loops_run` per iteration and `challenge_plan.applied` by the fixes each iteration lands. Concerns are counted in `challenge_plan.concerns` and follow the same rule as the spec challenger's above — foldable into an iteration already running, never a reason to loop. **`challenge_plan.applied_concerns` splits the total** the same way: a concern fix increments both `challenge_plan.applied` and `challenge_plan.applied_concerns`, a blocker fix increments `challenge_plan.applied` only, so blocker-driven fixes are `challenge_plan.applied - challenge_plan.applied_concerns`. The **errored-dispatch rule does not apply here** — it is scoped to the spec challenger; only the counter shape is shared. If blockers remain after the cap → STOP: surface them and require human input. **There is no scope-blocker bypass class** — unlike the spec challenger, all three plan lenses (spec-coverage, sequencing, interface-consistency) are text-fixable, so the single stop path is the only halt. This mirrors `dev:plan` Step 7a's matching rule.
...
**No stage is exempted from that rule here** — PR included, and PR is safely re-enterable. `dev:pr` Step 4 skips `gh pr create` when `state.json.artifacts.pr_url` is already set and reuses the stored PR number; Step 5c overwrites the decision log rather than writing a second one; Step 5d's `dev:reflect` replaces the existing `## Retrospective` rather than appending another; and the branch push runs on every path, so `dev:done` never merges a stale remote head. The rule is **idempotent resume, never a stop** — `dev:pr` states it — which is why re-entering PR adds nothing to the stop list below.
