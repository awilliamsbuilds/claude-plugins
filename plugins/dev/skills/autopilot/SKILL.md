...
**PR is the one stage re-entry cannot simply repeat.** `gh pr create` is not idempotent — a second create against the same branch fails, which would stop an unattended run at the last step. So if the run reaches PR with `artifacts.pr_url` already set, treat PR as satisfied and continue to Done rather than re-creating: the re-run stages pushed their commits to the same branch, so the open PR already carries them. This is the only named exception to the paragraph above, and it needs no new state — `artifacts.pr_url` is written by `dev:pr` Step 5 today.
