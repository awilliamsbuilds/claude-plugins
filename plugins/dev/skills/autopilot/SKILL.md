...
**Spec challenger — an errored dispatch is not an iteration.** In loop-bookkeeping terms: a dispatch that returns an error rather than a verdict does **not** advance `challenge.loops_run`, sets `challenge.blockers` / `challenge.concerns` to `null`, and is **retried once — once per stage, not once per round.** A second error STOPs. The per-stage scoping is the half that only matters here: standard mode never loops, so one retry per stage and one per round are indistinguishable there, while a deep-tier autopilot run reading it per-round would grant five retries instead of one. `dev:spec` Step 12a is **canonical** for this rule, including why it is not mode-split, how a standard-mode STOP is recovered, and why an errored dispatch is not the in-session fallback; only the bookkeeping half is restated here.
