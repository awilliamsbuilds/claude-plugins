...
**Borrow `dev:dev` Step 3's globs, not its multi-hit rule.** Where that skill lists several sessions and *asks* which to continue, autopilot may not — Step 2 forbids asking. So on more than one hit, STOP: "Found N in-progress cycles: `<feature>`, `<feature>`. Re-run naming the one you want: `/dev:autopilot docs/dev/<feature>/spec.md`." Picking one unattended would run Build, PR and merge on a cycle the user never named.
