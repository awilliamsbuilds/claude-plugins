...
9. Does any task introduce an auto-persist `useEffect` (or similar side-effecting hook) that depends on a value recomputed fresh each render — a new object, array, or Map instance rather than a primitive or memoized reference? If so, the task's implementation steps must explicitly call out in-flight/duplicate-fire protection (e.g. a ref tracking writes already in progress, or filtering out items already saved before re-triggering) — otherwise the effect can re-fire mid-save and produce duplicate concurrent writes before its own guard state has updated. Name the protection mechanism in the task, don't leave it implicit.
10. Does any task accept free-text numeric input (money, amounts, counts)? If so, exactly one task must explicitly own locale-aware parsing and formatting (comma-decimal locales, grouping separators, non-finite input) and name the shared helper or approach — never assume `Double(_:)`/`parseFloat`-style parsing. Locale handling left implicit ships silent input corruption that only surfaces in Validate or production.
11. Does any task name a framework API whose convention may have moved since training data — middleware, routing/file conventions, the DB driver, auth wiring, metadata/manifest? If the project flags its framework as diverging from training data (an `AGENTS.md`/`CLAUDE.md` note, or a pinned major version ahead of your knowledge), resolve the *installed* version's convention from its own docs (`node_modules/<pkg>/dist/docs/`, or the package's own types) **while drafting the task** and write the verified API into the implementation steps — do not leave a training-data guess for Build to correct. A task that names the wrong file, driver, or API is internally consistent and still wrong; catching it here costs a doc read, catching it in Build costs a task rewrite.
