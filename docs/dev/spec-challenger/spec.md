...
Runs on all tiers, scaling down for Micro — consistent with the skill's own anti-pattern note
that simple features are where unexamined assumptions cause the most wasted Build work.
...
obvious inline fixes. No double-running.

**6. State.** `state.json` gains a `challenge` block:

```json
"challenge": {
  "run": true, "blockers": 2, "concerns": 1,
  "applied": 2, "dismissed": 0, "loops_run": 0
}
```
...
**never** `spec_revisions` — reflect defines `spec_revisions` as post-gate churn *the human*
had to catch, and calls it "the strongest single signal." Routing challenger-caught fixes
through it would invert that meaning precisely when the feature is working. Separated, the two
counters form a diagnostic matrix:
...
7. `dev:reflect` surfaces `challenge.*` alongside `spec_revisions`.
8. `dev:spec` Step 11 no longer duplicates consistency, scope, ambiguity, or grounding.
9. With subagent dispatch unavailable, the checklist runs in-session and still produces a verdict.
...
*Grounding inventory: read `validate/SKILL.md` §2 → confirmed cold `general-purpose` subagent dispatch, explicit exclusion of conversation history, in-session fallback, P1–Nit classification, and `loops_max` by tier (micro 1 / standard 3 / deep 5); `find . -type d -name agents` → zero hits, repo is skills-only; read `spec/SKILL.md` Steps 11–13 → confirmed self-review items #1–#5 and the Step 13 revision loop (`spec_revisions`, `spec_end` re-stamp); read `fix/SKILL.md` → confirmed Linear issue text fetched via `mcp__linear-server__get_issue` pre-fills spec dimensions, making the injection guardrail load-bearing; read `autopilot/SKILL.md` → confirmed silent-backtrack convention and the stop-after-cap precedent for both confidence and validate; `grep -n "metrics" reflect/SKILL.md` → reflect extracts a hardcoded four-metric list (lines 38–42) and leans on `spec_revisions` as its primary signal (line 54), so it will NOT auto-detect a new `challenge` block; `grep -rl "config.json" plugins/dev/skills` → 8 readers (init, validate, shape, spec, start, dev, pr, autopilot), establishing the cost of a config toggle.*
