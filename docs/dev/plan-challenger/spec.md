...
- **`challenge_plan.*` state counters**, in a namespace separate from spec's `challenge.*`, so reflect's spec-quality reading (`challenge.blockers` paired with `spec_revisions`) is never corrupted. Counter shape mirrors spec's: `run`, `blockers`, `concerns`, `applied`, `dismissed`, `loops_run`, `loops_max`. **Correct in both modes** — every counter reaches its correct value in standard and autopilot with no counter's *non-default* autopilot value depending on a gate write, so this cycle does not reproduce the "gate-path write dead in autopilot" defect.
...
5. Every `challenge_plan.*` counter reaches its correct value in **both** modes: a counter that can hold a non-default value in autopilot has an autopilot-running writer (`applied` — the revision loop); a counter whose autopilot-correct value equals its init default may be written only in standard (`dismissed` — stays 0 in autopilot). No counter's *non-default* autopilot value may depend on a gate write.
...
- **Counter values must be correct in both modes** — this plugin has three recorded instances of the "gate-path write dead in autopilot" defect, and the last challenger-adding cycle reproduced it. The invariant is reachability, not write-location: no counter's *non-default* autopilot value may depend on a gate write. A counter whose autopilot-correct value is its init default (e.g. `dismissed` = 0) may be written only at the standard gate; a counter that takes a non-default autopilot value (e.g. `applied`, written by the revision loop) must have a writer on the autopilot path.
