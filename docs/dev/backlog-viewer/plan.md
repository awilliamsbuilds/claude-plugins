...
Run tests with `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s plugins/dev/skills/debt -p 'test_*.py'` from `$WORKDIR`. The env var is load-bearing, not decoration: `test_viewer.py` imports `viewer`, and on Homebrew or Linux Python that writes `plugins/dev/skills/debt/__pycache__/` into the repo — untracked files whose `.pyc` payloads embed absolute source paths, which would trip Task 9's portability grep. Apple's system `python3` happens to redirect bytecode to `~/Library/Caches`, so this never bites on the author's machine and would surface only for a contributor. Do not solve it by editing `.gitignore`.
...
- Produces:
  ...
  - the DOM contract Task 5 binds to, authored in full by this task — element ids `#repo`, `#count`, `#sort` (including both `<option>` elements, values `first_recorded` and `recurrence`), `#search`, `#rail`, `#clear`, `#list`, `#detail`, `#detail-resting`, `#detail-resting-count`, `#empty-filters`, `#empty-store`, `#empty-absent`; and the CSS classes `.chip`, `.row`, `.row.selected`, `.row.linked`, `.badge-parse-error`. Task 5 selects and populates these; it introduces no id or class of its own
...
7. Three **separately addressable** empty-state blocks, each hidden by default, copy verbatim from design. One shared container would leave Task 5 with no way to select among them:
   - `#empty-filters` — filters match nothing → `No items match these filters.` / `Clear a filter, or widen the search.`
   - `#empty-store` — `state == "empty"` → `No items in docs/backlog/ yet.` / `Capture one with /dev:debt add.`
   - `#empty-absent` — `state == "absent"` → `docs/backlog/ doesn't exist in this repo.` / `Run /dev:init to set up the backlog store.`
8. Static copy in the shell, verbatim from design: search placeholder `Search body, fields, file paths…`, the `Clear all` control, the rail's lowercase field headings `type` · `status` · `scope` · `severity` in that order, and the detail pane's resting state in `#detail-resting` — **both** of its lines, `Select an item to read it.` followed by `<n> items in the current filter.` with the count in `#detail-resting-count` (design.md:187 specifies two lines; carrying only the first drops the half that tells the operator what the filter is currently holding).
...
Interfaces:
- Consumes: `const STORE` in the page — `{state, repo_name, total, items, facets}`, where each item has `id`, `archived`, `fields`, `body`, `parse_error`, `raw`, `related` (`null` or `{id, resolved, archived}`), `search`; and each facet entry has `value` (`string` or `null`), `label`, `count`. Also the DOM contract Task 4 authors: `#repo`, `#count`, `#sort` (with its two `<option>`s already present), `#search`, `#rail`, `#clear`, `#list`, `#detail`, `#detail-resting`, `#detail-resting-count`, `#empty-filters`, `#empty-store`, `#empty-absent`, `.chip`, `.row`, `.row.selected`, `.row.linked`, `.badge-parse-error`. This task adds no new id or class
...
Implementation steps:
1. On load: set `document.title` to `dev backlog — <repo_name>` and `#repo`'s text from `STORE.repo_name`; render the rail from `STORE.facets`; render the list; select nothing, leaving `#detail-resting` showing.
...
6. Header count renders `<n> of <total>` always — both numbers, so an active filter is never invisible. On every re-filter also update `#detail-resting-count` with the same `<n>`, so the detail pane's resting line stays true to the current filter rather than to the load-time set.
...
13. Empty states, selected by id: `STORE.state === "absent"` → show `#empty-absent`; `"empty"` → show `#empty-store`; otherwise an empty filtered set → show `#empty-filters`. The store-level states win over the filter state and render no rail options. Exactly one of the three is ever visible.
...
Interfaces:
- Consumes: the full CLI from Task 7 (`start` / `stop` / `serve --primary --port`) and the test suite from Tasks 1–3 and 6–7. Task 8's `SKILL.md` is read for correctness but **not executed** — the installed plugin is a snapshot of `main`, so the `view` verbs are not live until Stage 7's merge plus `/plugin update`
...
4. **SC4 and SC7 together — worktree parity and idempotency, verified at the CLI layer.** Run `python3 "$WORKDIR/plugins/dev/skills/debt/viewer.py" start` with the cwd set to the cycle worktree; note the URL and pid. Run the identical command with the cwd set to `$PRIMARY`: it must print the **same** URL, spawn nothing, and report the same pid. Same URL from both cwds is simultaneously the parity proof and the idempotency proof.
   **Do not invoke `/dev:debt view` here.** The installed `dev` plugin loads from a marketplace snapshot, not from this worktree — verified: the installed `skills/debt/` directory contains `SKILL.md` alone, with no `view` verb and no `viewer.py`. Per CLAUDE.md, Task 8's edit goes live only after the merge plus `/plugin update`, which is Stage 7. Verifying through the wrapper would mean depending on an artifact this cycle has not shipped yet.
   Task 8's step is a thin forwarder — it resolves a path and prints the script's stdout verbatim, holding no logic of its own — so the CLI layer is where the behavior actually lives. Report SC4/SC7 as **verified at the CLI layer**, and note that the `/dev:debt view` wrapper is re-confirmed after merge and `/plugin update`. Step 8's proxy rule applies: this is reported as a proxy, not as the criterion.
...
6. **SC6 — portability.** `grep -rn '/Users/\|awilliamsbuilds\|adam' plugins/dev/` must return zero hits. Run it after every file is written, including the test file and the HTML template, and after the test suite has run at least once — a stray `__pycache__/` would put absolute source paths under `plugins/dev/` and turn this green check red.
7. Stop the fixture server and the viewer when done: `python3 "$WORKDIR/plugins/dev/skills/debt/viewer.py" stop` must report `Backlog viewer stopped.`, and a second `stop` must report `No backlog viewer is running for this repo.` and exit `0`. Same reason as step 4 — the wrapper isn't installed yet.
