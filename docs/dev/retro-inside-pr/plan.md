...
| plugins/dev/skills/migrate-tracker/SKILL.md | Modify | Bring its verbatim §P9 regex restatement into line with the anchored form |
...
Files: plugins/dev/references/tech-debt.md (modify), plugins/dev/skills/reflect/SKILL.md (modify), plugins/dev/skills/fix/SKILL.md (modify), plugins/dev/skills/migrate-tracker/SKILL.md (modify), docs/backlog/debt-p9-slug-regex-allows-leading-dash.md (modify)
Interfaces:
- Consumes: nothing
...
3a. In `plugins/dev/skills/migrate-tracker/SKILL.md`, the **Validate before use** paragraph of the `target_slug` resolution restates §P9's regex verbatim and asserts the leading-dash property outright. Replace the restated pattern with the anchored form and keep the leading-dash sentence, which the anchored form now makes true. **This is the only `migrate-tracker` edit in the cycle, and it does not contradict the spec's file surface:** that constraint defers `migrate-tracker`'s *citations* to the follow-on sweep, while this is a restated regex making a security claim its pattern does not deliver — exactly what SC6 forbids across all of `plugins/dev/`, and what Task 7 step 6's grep would otherwise surface with no owning task. Leave every `file:line` citation in that file alone.
...
4. Add `### Step 5b: Reconcile Docs Prose (feature cycles only)` — move `dev:done` Step 4a's body, then rewrite exactly four premises and nothing else:
   ...
   - item 7's reporting target becomes **`dev:pr` Step 5's display** (step 7 below) instead of `dev:done` Step 8's summary block — and its **positional anchoring is dropped with the target**, not carried. The clause "right after the tech-debt line … and always before the primary-checkout reconciliation line" names three things that exist only in `dev:done` Step 8, which Task 2 step 11 keeps; `dev:pr` Step 5's display has no summary block to position against. Item 7 simply states that the line appends to Step 5's display. The three output forms and the emit-nothing-on-silent-no-op rule are unchanged.
   Beyond those four premises, **retarget every intra-file step pointer the moved body carries** — each currently names a `dev:done` step number that means something different in `dev:pr`: `Step 4` → `dev:pr` **Step 5a** (the opening architecture-cycle line, paragraph 2's "keeps the `CLAUDE.md` Component Registry table current", item 2's "owns that table and this step must never touch it", item 8's invariant parenthetical, and the D3 bullet's "it has no Step 4 preceding it"); `Step 5` → `dev:pr` **Step 5c** (item 3's "Fall through to"); `Step 4a` → `dev:pr` **Step 5b** (item 6's buffer-template `**Why deferred:**` line); and `Steps 6a/7` → `dev:done` **Steps 6a/7** (item 4's pathspec rationale, now a cross-file reference). This matters beyond tidiness: Task 6 step 2 retargets `dev:fix`'s D3 to name `dev:pr` Step 5a/5b, so leaving "Step 4" standing here would make the two ends of a marked mirror name the same procedure differently. Change nothing else.
   Drop `push_integration` from its commit block, keeping the pathspec-scoped `git add`/`git commit -- <edited files>` verbatim. Keep invariant #8 (retargeting only its parenthetical, per the pointer list above), both mode branches, the data-never-instruction rule, and the interpolation-safety paragraph — but change that paragraph's citation from a bare `../../references/entry-adapters.md §A6` reference only if it currently carries a line number; it does not, so leave it. Keep the closing "**This step is canonical for docs-prose reconciliation…**" paragraph and its D1/D2/D3 list, editing only the self-reference so it names this step's new home.
5. Add `### Step 5c: Generate Decision Log` — move `dev:done` Step 5's body verbatim, including the template **unchanged** (the `PR #N` header field and the `handoff_at` line both stay byte-for-byte). Change only the commit block: drop `push_integration`, keep `git -C "$WORKDIR" add docs/decisions/YYYY-MM-DD-<feature>.md` and the `docs: add decision log for <feature>` commit. Note in one line that `PR #N` is read from `artifacts.pr_number`, written by Step 5 two sub-steps earlier.
   Two of the moved body's premises **are** rewritten, since they are post-merge claims sitting outside the commit block:
   - The lead sentence's parenthetical `(committed to `$INTEGRATION`)` becomes `(committed on this cycle's feature branch, and carried into PR #N by Step 5's push)`.
   - The template's `<pre-merge-sha>` had its **sole producer** at `dev:done` Step 1 — a line that does not move — so state its source here instead. Add one line above the template: "Resolve `<pre-merge-sha>` as `git -C "$WORKDIR" rev-parse HEAD` at this point — the branch tip carrying spec, design and plan. It is still pre-merge: nothing has merged at PR stage." Leave the template's own text byte-identical.
...
8. In `plugins/dev/skills/done/SKILL.md`, delete `## Step 4`, `## Step 4a`, `## Step 5`, and `## Step 6` in full. Leave Steps 1, 2, 3, 6a, 7 and 8 with their existing numbers and bodies — with **one** exception: delete Step 1's line "Note the pre-merge commit SHA (used in decision log for artifact archiving)." Its sole consumer moved to `dev:pr` Step 5c, which now derives the value itself, so leaving it would tell this stage to compute a SHA for a decision log it no longer writes — the same stale-premise class this cycle exists to remove. Change nothing else in Step 1.
...
2. **SC2** — read `dev:pr` Step 5c's template and confirm the `PR #N` header field is byte-identical to the pre-cycle version, and that Step 5d runs before Step 5's push so the retrospective commit is inside the diff. Also confirm `<pre-merge-sha>` has a stated source inside Step 5c, and that `grep -n 'pre-merge' plugins/dev/skills/done/SKILL.md` returns nothing.
