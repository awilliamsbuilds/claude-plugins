...
- debt-cross-file-line-citations-go-stale-silently — folded in because this cycle breaks three live `file:line` citations outright (`reflect/SKILL.md:205`, `reflect/SKILL.md:212`, `done/SKILL.md:504`) by renumbering `dev:done` Steps 4/4a/5/6 and adding steps to `dev:pr`. Recurrence 2. Paying it means converting `file:line` citations to stable anchors across the `dev` plugin rather than repairing only the three this PR happens to break.
