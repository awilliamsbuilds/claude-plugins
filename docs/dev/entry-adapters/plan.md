...
5. **§A4 — The backlog adapter.** Four subsections:
   ...
   - **Closeout.** Fires in the tail after `delete_feature_branch` returns 0. **This is a mirror of `dev:debt` Step 6 step 4 — the write and the P3 move — which is canonical.** Restate that step's branch structure in full here rather than pointing at it:
     1. **Preconditions, both checked before any write.** (i) `RECONCILED=1` — if reconciliation was skipped (detached checkout, or a `pull --ff-only` that did not apply), **skip the closeout entirely**, leave the item open, and say so in the tail's Report; a close committed on a detached HEAD or a stale default branch reaches no branch. (ii) The resolved `$PRIMARY/docs/backlog/$ITEM.md` has front-matter `status: open` — a coincidental basename match must never archive an unrelated or already-closed item. The realistic route to one is a Linear `gitBranchName` that happens to begin `fix/` (§A3's `..` and `//` rejections already close the traversal case), which is exactly why the check is cheap insurance rather than paranoia.
        ...
        **Guard what the resolution block binds; re-derive what the merge fence binds.** This block is separated from the merge fence by a front-matter edit, which forces an agent turn and so almost certainly runs in a *new* shell invocation. `fix/SKILL.md:477-484` states the hazard for its own variable set — an unbound numeric makes `[ "$X" -eq 1 ]` error and evaluate false, **silently skipping** the guarded action, and `git -C ""` silently operates on the current directory rather than failing. The same hazard applies here, but the two variable classes need opposite treatment:

        - `PRIMARY`, `DEFAULT_BRANCH`, `BRANCH`, `ITEM` — all bound by the tail's **resolution block** (`fix/SKILL.md:391-418`) and re-derivable by re-running it. Assert them with `:?`. **`ITEM` must be derived there, alongside `BRANCH`** — deriving it inside the block that guards it would abort the closeout on every run.
        - `RECONCILED` — bound at `fix/SKILL.md:524`, **inside the merge fence**, which is precisely the invocation this block is *not* in. Asserting it with `:?` would abort every run. Re-derive it from observable state instead:
        ...
        if [ "$(git -C "$PRIMARY" branch --show-current)" = "$DEFAULT_BRANCH" ] \
           && git -C "$PRIMARY" merge-base --is-ancestor "origin/$DEFAULT_BRANCH" HEAD 2>/dev/null; then
          RECONCILED=1
        fi
        ```

        A detached checkout fails the first test and a non-fast-forwarded one fails the second — exactly the two `RECONCILED=0` states the merge fence produces. Re-deriving beats inheriting here regardless of shell lifetime: it reads the state rather than trusting a variable, which is the same rule the tail's Report section already imposes on itself.

        **State the re-derivation's two known edges rather than implying it is exact.** It reads a *fetched* ref, so (i) if `pull --ff-only` failed at its fetch stage, `origin/$DEFAULT_BRANCH` is stale at the pre-merge tip, `--is-ancestor` returns true, and an unreconciled checkout reads as reconciled — which degrades into step 4's documented edited-but-unpushed report rather than corrupting anything, and is largely unreachable anyway because `delete_feature_branch`'s own `gh pr view` fails closed on the network causes first; and (ii) in a clone with no `origin/$DEFAULT_BRANCH` ref, `merge-base` errors and a legitimate close is skipped — carry `fix/SKILL.md:397`'s `rev-parse --verify` existence fallback to close that one.
...
**Files:** modify `plugins/dev/skills/fix/SKILL.md` (Step 1 Reads block; new Step 2a; Steps 5, 6, 7 — including the tail's resolution fence at `:391-418`, which step 4a's leftover-scan note touches; see the Task 4/6/7 concurrency warning)
**Interfaces:**
...
4a. **Unhardcode the `fix/` prefix — three sites, and without this the Linear path is broken, not degraded.** Step 5 binds the resolved name to `BRANCH_NAME`, whose value is `<gitBranchName>` on the linear dispatch, `fix/<item>` on the backlog dispatch, and `fix/<kebab-summary>` on free text. Linear branch names are conventionally `<user>/<id>-<title>` and carry no `fix/` prefix, so every literal must become the variable:
   - `fix/SKILL.md:303` — `push -u origin "fix/<kebab-summary>"` → `push -u origin "$BRANCH_NAME"`.
   - `fix/SKILL.md:320` — `--head "fix/<kebab-summary>"` → `--head "$BRANCH_NAME"`.

   `BRANCH_NAME` is bound in Step 5's fence and consumed in Step 6's PR fence, crossing the whole Change/Verify span — the same boundary `$PRIMARY`, `$SLUG`, and `$DEFAULT_BRANCH` already cross at these exact lines, so this adds no new hazard class. Say so in one clause rather than leaving it unremarked, since the closeout block reasons explicitly about the identical risk; if the PR fence is ever given a `:?` bind guard, `BRANCH_NAME` belongs in it.
   - `fix/SKILL.md:399` — the tail's leftover scan globs `refs/heads/fix/*`, which by construction cannot match a Linear-sourced branch. **Keep the `fix/*` scoping** — the file's own prose at `:437-443` justifies it as what stops the scan listing colleagues' branches on a repo using `fix/` as a team convention, and widening it would resurrect exactly that. Instead add one sentence naming the resulting gap: an interrupted tail on a **Linear-sourced** branch is not listed by this scan, so the `$DEFAULT_BRANCH` guard falls through to its flat message. The file calls that class of miss permanent rather than transient (`:445-453`), so it is named here rather than left for a reader to discover.
5. In **Step 6 (PR)**, add `Closes [<ID>](<url>)` to the body. `fix/SKILL.md:350` announces "**PR body — four required sections**" directly above the template — the `Closes` line is a lead line, not a fifth section, so leave the count at four and say so in one clause. Task 9 makes the matching statement on the canonical side. It goes inside the single-quoted heredoc like every other body line — the ID and URL are external input from Linear and must not reach a double-quoted `--body`. Add it as its own line above `## What changed` so Linear's parser sees it regardless of body length.
...
**Files:** modify `plugins/dev/skills/fix/SKILL.md` (Step 2a; Step 3; Step 5; Step 7 — including the tail's resolution fence at `:391-418` where `ITEM` is bound, and the mirror paragraph at `:576-581`)
**Interfaces:**
...
5. In **Step 7 (the tail)**, add the **Closeout** hook after `delete_feature_branch` returns 0, implementing §A4's four numbered steps in full and in order: the two preconditions (`:?`-guard the four resolution-block variables, **re-derive** `RECONCILED` rather than asserting it, then require `status: open`), the front-matter edit, the P3 move, and the pathspec'd commit-and-push modeled on `dev:done` Step 6a. Trace the sequence end-to-end before writing it: `delete_feature_branch` runs after the checkout+pull block, so `RECONCILED` is already bound and the checkout is on `$DEFAULT_BRANCH` at the merged tip — both prerequisites the commit needs are satisfied at that point, and nowhere earlier.
...
8. **Update the tail's "deliberately absent" list at `fix/SKILL.md:576-581`.** It currently reads that `push_integration` is absent "(the lane makes no post-merge commits, so it never pushes to the integration branch)" — which this task falsifies verbatim, in the same file, inside the two-ended `dev:done` Step 2 mirror pointer. **Replace only the parenthetical**, which already follows the clause "and its `push_integration` helper" — so the replacement text starts at the em dash, or the result reads "and its `push_integration` helper and its `push_integration` helper". New parenthetical: *"— the lane targets `$DEFAULT_BRANCH` directly and has no `$INTEGRATION` branch, so the backlog closeout's post-merge commit reuses that helper's fetch/rebase/re-push **shape** without the helper itself."* Leave the detached-HEAD-worktree-path entry unchanged.
9. **Name SC3 and how this reads against it.** SC3 requires the item be "closed via the existing close path, not a second implementation," and spec Technical Constraints requires reusing `dev:debt` Step 6 or the `debt-pending.md` buffer rather than adding a third way. Neither is literally invocable here: Step 6 requires a user confirmation turn and refuses to commit, and the buffer is flushed by `dev:done`, which the lane never enters (it writes no `state.json`). A **marked mirror that restates the canonical in full and names all three divergences** is the closest available reading — it keeps one canonical, makes the second site findable from the first, and is the pattern this repo already uses for `dev:pr` Step 4 / `dev:fix` Step 6. State this in the skill so a validator reading SC3 literally does not read Task 4 as a violation.
...
**Depends on:** nothing — independent of Tasks 1–5. **Do not run this concurrently with Task 7, Task 4, or Task 3.** All four edit the *same* fenced block (`fix/SKILL.md:391-418`), not adjacent ones — the lines do not overlap, but two concurrent rewrites of one fence clobber each other. Run them back to back in either order.
...
**Depends on:** nothing — independent of Tasks 1–5. **Do not run this concurrently with Task 6, Task 4, or Task 3** — all four edit the same fenced block (`fix/SKILL.md:391-418`); run them back to back.
**Files:** modify `plugins/dev/skills/fix/SKILL.md` (Step 7, "Resolve the branch and PR")
**Interfaces:**
- Consumes: nothing
...
5. Add `entry-adapters` to the item's `cycles:` list only if the store contract treats a bookkeeping touch as a recurrence — check `references/tech-debt.md` **§P6** before deciding — the append rule lives there ("On a clear match: append this cycle's name…"), not in §P8, which is the recurrence *ranking*. §P6 requires the same defect re-found, so the answer is to leave `cycles:` alone, and leave it alone if the contract scopes `cycles:` to recurrences of the finding.
