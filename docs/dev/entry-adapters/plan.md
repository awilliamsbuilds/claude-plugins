...
**Interfaces:**
- Consumes: nothing
- Produces: section anchors `§A1`–`§A6` (below), cited by name from every consuming task
- State keys: none
...
4. **§A3 — The Linear adapter.** Six subsections:
   ...
   - **Branch name.** `<gitBranchName>` is external input reaching `git checkout -b`. Validate it against the branch-name allowlist before use; on failure fall back to the lane's own `fix/<kebab-summary>` derivation rather than refusing the work (spec Edge Cases). The lane's Step 5 collision check runs on the resolved name either way.
...
5. **§A4 — The backlog adapter.** Four subsections:
   ...
   - **Closeout.** Fires in the tail after `delete_feature_branch` returns 0. **This is a mirror of `dev:debt` Step 6 steps 1–4, which is canonical** — restate its branch structure in full here rather than pointing at it:
     ...
     4. Stage, commit, and push — **modeled on `dev:done` Step 6a's flush commit (`done/SKILL.md:441-453`), which is the established shape for a post-merge store write.** Use a `docs/backlog/` pathspec on both the `--cached --quiet` check and the commit, so the commit cannot sweep in anything else; use `git commit -F -` with a single-quoted heredoc; and push with the same fetch/rebase-retry shape `push_integration` uses (`done/SKILL.md:128-133`). A push that still fails after the retry leaves the item edited-but-unpushed: report it and name the file, rather than exiting silently.

     **Why this commits where the canonical does not.** `dev:debt` Step 6 step 5 refuses to commit, and gives its reason: it "is invoked outside a cycle, usually with the primary checkout sitting on `main`, and the standing convention is never to commit directly to `main`." Neither half holds here — the tail is inside the lane's own flow and has just merged the user's own PR, so this is the merge's bookkeeping, not an unrelated change to the default branch. That is precisely the class of write `dev:done` Step 6a already makes post-merge. Record the divergence and this grounding in both files; Task 10 carries the other end. Do **not** "fix" either side into agreement with the other.
...
**Interfaces:**
- Consumes: `linear` dispatch + identifier (Task 2); §A1's hook table and §A3's six subsections (Task 1)
- Produces: `<ID>`, `<url>`, `<teamId>`, resolved status IDs, and the validated branch name — all in-turn values, nothing persisted
- State keys: none — the lane writes no `state.json` (`fix/SKILL.md:16`)
...
3. In **Step 5 (Branch)**, add the Linear branch-name branch: use `<gitBranchName>` when it passes the allowlist; otherwise fall back to the existing `fix/<kebab-summary>` derivation. Run the existing local+remote collision check on whichever name resolved, unchanged.
...
**Files:** modify `plugins/dev/skills/fix/SKILL.md` (Step 2a; Step 3; Step 7)
**Interfaces:**
- Consumes: `backlog` dispatch + slug (Task 2); §A4 (Task 1)
- Produces: request text, grounding hints (the item's `files:` list), display label `<type>-<slug>`
- State keys: none
- Shared procedure: the close write is a **mirror of `dev:debt` Step 6 steps 1–4** (canonical). §A4 restates that procedure's full branch structure — the `RECONCILED=1` gate, the status/date/closed_by edit, the P3 move, `closed/` create-if-absent, and the commit/push — and names the one deliberate divergence (this mirror commits and pushes; the canonical does not), grounded in `dev:done` Step 6a's matching post-merge store write rather than in fresh reasoning. Task 10 carries the matching back-pointer.
...
2. Note that this adapter has **no external dependency** — a missing or unauthenticated Linear MCP never reaches this path (spec SC7).
3. In **Step 3 (Ground)**, state that the item's front-matter `files:` are read first as grounding hints, and that they are hints rather than a boundary — the existing rule that a named set is enumerated by sweep rather than recall still governs.
4. In **Step 7 (the tail)**, add the **Closeout** hook after `delete_feature_branch` returns 0, implementing §A4's four numbered steps in full and in order: the `RECONCILED=1` precondition (skip and leave open otherwise), the front-matter edit, the P3 move, and the pathspec'd commit-and-push modeled on `dev:done` Step 6a. Trace the sequence end-to-end before writing it: `delete_feature_branch` runs after the checkout+pull block, so `RECONCILED` is already bound and the checkout is on `$DEFAULT_BRANCH` at the merged tip — both prerequisites the commit needs are satisfied at that point, and nowhere earlier.
5. Add the closeout's outcome to the tail's Report — closed and pushed, or left open with the reason.
6. State that the tail knows it is backlog-sourced by the presence of the item file matching the branch's work, and that a plain `/dev:fix merge` on a free-text branch runs no closeout — the hook is a no-op, not an error.
...
2. At the end of Step 6, add the two-ended pointer: `references/entry-adapters.md` §A4 mirrors steps 1–4 of this procedure for the `dev:fix backlog` tail, and a change here should be reflected there.
3. State the one deliberate divergence and its reason, in the terms step 5 already uses. Step 5's stated reason for not committing is that `dev:debt` runs outside a cycle with the checkout usually on the default branch, where the standing convention forbids a direct commit. The mirror commits because neither half holds there: it runs inside the lane's tail on a checkout that has just merged the user's own PR, which is the same post-merge bookkeeping write `dev:done` Step 6a makes (`done/SKILL.md:441-453`). Extend step 5's existing "Do not 'fix' this by adding a commit" to say the mirror's commit is not the thing being warned against — otherwise that sentence reads as forbidding §A4.
4. Sweep this file's `dev:linear` reference (line 31's "external Linear issue" provenance) as part of Task 11's sweep, not here.
...
2. Change "All 13 sites carry a non-empty check" to "All 12 sites" in `**Done looks like:**`, and update the `grep -rn 'PRIMARY=' plugins/dev/skills/*/SKILL.md` sentence if it names a count.
...
3. **SC10** — `git diff --stat origin/main -- plugins/dev/skills/{shape,build}/SKILL.md plugins/dev/skills/autopilot/SKILL.md plugins/dev/skills/dev/SKILL.md` — `shape`, `build`, and `autopilot` must be **empty**; `dev`, `plan`, `validate`, `done` must show only rename-reference lines. Read the actual diff, do not assert it.
4. **SC11** — `grep -c '^  - plugins/' docs/backlog/debt-primary-cd-failure-unchecked.md` must return 12, and the body's site count must agree.
...
- **`Closes [<ID>](<url>)` depends on Linear's GitHub integration parsing the PR body.** This is Linear-side behavior no repo file can assert. It is not verifiable from this repo, and Task 13 step 6 must record it as *not executed* rather than implying otherwise. If it turns out Linear only parses the branch name, the `gitBranchName` branch (Task 3 step 3) already carries the ID and provides the fallback path — the design does not have a single point of failure here.
- **This cycle sits at the top of one deep cycle's range** (spec's Note for Plan). 13 tasks across 17 files. The task list came back at the size the spec predicted rather than above it, so the fallback split is **not** invoked. If Build finds Tasks 1–5 growing past `fix/SKILL.md`'s working size, the spec's source-shaped split is the ordering to fall back to: (1) Tasks 1, 2, 4, 6, 7 — the seam, the backlog adapter, and both `debt-fix-tail-*` items, all offline-testable; then (2) Tasks 3, 5, 8, 9, 11, 12 — the Linear adapter and the deletion, which is forced by it.
...
- **Cross-skill behavior ripple — checked, no change needed.** Task 3 and Task 8 add new STOP conditions, which would normally require updating `dev:autopilot` Step 2's "When autopilot stops" list. Verified against `autopilot/SKILL.md:23-31`: that list enumerates stops inside the seven-stage cycle. `dev:fix` is not a stage and is never invoked by autopilot. `dev:spec`'s new STOP is reachable only via the `linear <issue-id>` argument form, which autopilot never constructs — it resumes with an artifact path or begins from a free-text request (`autopilot/SKILL.md:36-40`). So `dev:autopilot` stays byte-identical, which is also what spec SC10 requires. Recorded here because the check is what makes the omission deliberate rather than missed.
