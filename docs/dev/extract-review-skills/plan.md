...
- Shared procedure (two): **`<tree>` resolve-and-validate** (allowlist → `rev-parse --is-inside-work-tree` → stop, never fall back) — this task is the **canonical** implementation, and Task 4 (`dev:secure` Step 2a) is a marked mirror of it. **Empty-diff reporting** (say the diff is *empty*, name tree and base, never "no findings") — here the direction reverses: `dev:secure`'s existing rule (`secure/SKILL.md:250-255`) is **canonical**, and step 7 below writes a marked **mirror** of it into `dev:review`. Both ends carry the marking; a change to either must be reflected at the other.
...
3. **Bind and validate `<tree>` — canonical implementation, and it comes first.** `<tree>` must be resolved before `<base>` is verified, because the base is verified *in* the tree (step 4) — validating the base first would reference an unbound `$TREE`, and it would also give the wrong refusal: a mistyped tree would surface as "base does not resolve" rather than as a named tree refusal. Write the full branch structure, since Task 4 must restate it:
...
4. **Bind and validate `<base>`, in the tree step 3 resolved.** Reuse the pattern `dev:secure` already uses (`secure/SKILL.md:172`): `^[A-Za-z0-9._][A-Za-z0-9._/-]*$`, then `git -C "$TREE" rev-parse --verify --quiet "$BASE^{commit}"`. Note in-file that a bare SHA passes both gates unchanged, so a caller may hand a SHA rather than a branch name (measured at spec time against `git rev-parse HEAD`).
...
7. **Empty diff** → say the diff is **empty** and stop. Do **not** report "no findings" — that reads as a review that ran and came back clean, a different claim from one that had nothing to examine. Name the tree and the base in that message. **Mark this a mirror of `dev:secure`'s canonical empty-diff rule (`secure/SKILL.md:250-255`)** — restate its content in full rather than pointing at it, and note that a change to either side should be reflected at the other. Task 4 step 9 carries the matching pointer from the canonical end.
...
**Depends on:** Task 1 (the file and `## Step 1`'s dispatch to this section) and Task 2 (the absolute-path allowlist `^/[A-Za-z0-9._][A-Za-z0-9._/-]*$`, which this mode reuses to validate `<paths>`).
**Files:** modify `plugins/dev/skills/review/SKILL.md`
**Interfaces:**
- Consumes: from Task 1 — the file, the `docs` mode name, and `## Cold dispatch`; from Task 2 — the absolute-path allowlist `^/[A-Za-z0-9._][A-Za-z0-9._/-]*$`.
...
- Shared procedure (two): **`<tree>` resolve-and-validate** — this task is a **mirror of Task 2**, which stays canonical. Per the Isolation Principle this task restates Task 2's branch structure in full rather than pointing at it. **Empty-diff reporting** — this file holds the **canonical** statement (`secure/SKILL.md:250-255`, which step 9 below updates to name the tree); Task 2 step 7 is the marked mirror. A change to either side of either pair must be reflected at the other, and both ends say so.
...
4. **Write the `<tree>` resolve-and-validate branch structure in full** (mirror of Task 2, restated, not referenced):
   ...
   - **Order the two resolutions tree-first**, mirroring Task 2 step 3: resolve and validate `<tree>` before the base's `rev-parse --verify` check, then run that check as `git -C "$TREE" …` so the base is verified in the tree it will be diffed in. Base-first would reference an unbound `$TREE` and would report a mistyped tree as "base does not resolve". Note that this reorders `secure/SKILL.md:154-178` — token *binding* order is unchanged (base is still token 2), only the validation order moves.
...
9. **Update the empty-diff message (`:250-255`)** to name the **tree** alongside the branch and base, since the commonest cause of a surprising empty diff is auditing a different tree than intended. **This is the canonical statement of the empty-diff rule**; Task 2 step 7 carries a marked mirror of it in `dev:review`. Add the pointer here, so a later editor of this passage knows there is a second site to keep in step.
...
**Depends on:** Tasks 1, 2, 3 (both reviewer modes must exist with all eleven moved bullets and the severity table before validate deletes its copies), Task 4 (`dev:secure` must accept a tree before validate passes one), and Task 5 (step 2's rationale for the new parallelism shape turns on the subagent already living inside `dev:secure`).
**Files:** modify `plugins/dev/skills/validate/SKILL.md`
**Interfaces:**
- Consumes: from Task 2 — `/dev:review diff <base> [<tree>] [<artifact-path>…]` with positional binding; from Task 3 — `/dev:review docs <paths>`; from Task 4 — `/dev:secure diff <base> [<tree>]`; from Task 5 — `dev:secure`'s internal cold dispatch, which is what makes Step 2's parallelism a property of two skill invocations rather than of one dispatch site.
...
7. **Extend `not run` → stop to both reviewers** (`:639-643`): only when a review genuinely cannot run — the skill is unavailable, or no base ref resolves — does its result become `not run — <reason>`, and on that value **the lane stops** rather than opening a PR with a missing review. "Could not run" is a stop, never a pass-through. Bind the code-review half to `CODE_REVIEW_RESULT`.
   **State the `not run` disambiguation outright, because the phrase names two different things with opposite consequences.** Task 2 step 9's `not run` is a **per-bullet finding status inside a delivered report** — it appears on *every* lane run, since the lane passes no artifact paths — and it never sets `CODE_REVIEW_RESULT=not run` or stops anything. This step's `not run — <reason>` is a **whole-review outcome**, reached only when the review could not be performed at all. A report that arrives with two bullets marked `not run` is a review that ran.
...
**Depends on:** Tasks 1 (`## Cold dispatch` must exist), 2 (its `diff` mode must own the plan-coverage bullet before `plan:201`'s first clause points at it), 8 (the citations go stale only when Step 2's checklists go), 6 (step 7 below verifies `secure:123`'s rewrite landed), and 11 (step 9 below verifies `fix:652` and `fix:677`).
...
| Bare SHA as `<base>` | Task 2 step 4, Task 8 step 1 | Passes the existing allowlist and `rev-parse --verify` unchanged — measured at spec time; no allowlist edit needed |
