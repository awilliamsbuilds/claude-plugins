...
4. `dev:validate` Step 2 contains **none** of its three review checklists and **no** Architecture
   ...
   **scoped to `plugins/dev/skills/validate/SKILL.md`** returns nothing, while the same greps run
   repo-wide find **every one of them**: the six code bullets and the five document-review bullets in
   `review/SKILL.md`, the Architecture severity rows in its `docs` mode, and the five security bullets in
   `secure/SKILL.md`'s Pass C — the security bullets land in `dev:secure`, not `dev:review`, which is why
   "`dev:review` carries all of them" would be the wrong test. Moved, never dropped: the standard
   Criterion 12 sets for the config-contract bullet, applied to all eleven. Outside Step 2, `dev:validate`'s
