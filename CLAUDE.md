...
| `dev:fix` | plugins/dev/skills/fix/SKILL.md | The fast path — grounded change from request to open PR in one unattended run, with no cycle artifacts; escalates to `/dev` at 2+ unresolved decisions. `/dev:fix merge` is the deliberate second invocation that merges and cleans up. Mirrors `dev:pr` Step 4 and `dev:done` Step 2/7, which stay canonical |
| `dev:linear` | plugins/dev/skills/linear/SKILL.md | Linear-aware entry point into the /dev workflow |
