...
| `dev` | `dev`, `dev:init`, `dev:start`, `dev:spec`, `dev:shape`, `dev:plan`, `dev:build`, `dev:validate`, `dev:pr`, `dev:done`, `dev:reflect`, `dev:fix`, `dev:autopilot`, `dev:debt`, `dev:migrate-tracker` | Structured multi-stage development workflow (spec → shape → plan → build → validate → PR → done), plus `dev:fix` — the fast path that goes straight to an open PR with no cycle artifacts |
